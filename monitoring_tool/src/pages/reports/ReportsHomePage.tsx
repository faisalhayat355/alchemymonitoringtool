...
import ReportsAvailibilityPopup from "./reportavailabilitypop-up/AvailabilityStep1";
  ...
  return (
   ...
   <Grid item xs={7.32} mt={0.5}>
         <Grid container  sx={{display:'flex',justifyContent:'flex-end',alignItems:'center'}}>
          ...
          <Grid item xs={3}>
          <select style={{width:'75.3%'}}>
           <option>Hostgroup(s)</option>
           <option>Host(s)</option>
           <option>Servicegroup(s)</option>
    ...
    <ReportsAvailibilityPopup handleClose={handleClose} modal={modal}/>
   </>
  );
};
